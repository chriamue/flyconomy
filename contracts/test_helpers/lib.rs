...
macro_rules! balance_of {
    ($client:ident, $address:ident, $account:ident) => {{
        let _msg =
            build_message::<ContractRef>($address.clone()).call(|contract| contract.balance_of(address_of!($account)));
        $client
...
macro_rules! owner_of {
    ($client:ident, $address:ident, $id:expr) => {{
        let _msg = build_message::<ContractRef>($address.clone()).call(|contract| contract.owner_of($id));
        $client
            .call_dry_run(&ink_e2e::alice(), &_msg, 0, None)
            .await
            .return_value()
...
macro_rules! mint_dry_run {
    ($client:ident, $address:ident, $account:ident, $id:expr) => {{
        let _msg =
            build_message::<ContractRef>($address.clone()).call(|contract| contract.mint(address_of!($account), $id));
        $client
            .call_dry_run(&ink_e2e::alice(), &_msg, 0, None)
            .await
            .return_value()
...
macro_rules! get_role_member_count {
    ($client:ident, $address:ident, $role:expr) => {{
        let _msg =
            build_message::<ContractRef>($address.clone()).call(|contract| contract.get_role_member_count($role));
        $client
            .call_dry_run(&ink_e2e::alice(), &_msg, 0, None)
            .await
            .return_value()
...
macro_rules! get_role_member {
    ($client:ident, $address:ident, $role:expr, $index:expr) => {{
        let _msg =
            build_message::<ContractRef>($address.clone()).call(|contract| contract.get_role_member($role, $index));
        $client
            .call_dry_run(&ink_e2e::alice(), &_msg, 0, None)
            .await
            .return_value()
...
macro_rules! get_shares {
    ($client:ident, $address:ident, $user:ident) => {{
        let _msg = build_message::<ContractRef>($address.clone()).call(|contract| contract.shares(address_of!($user)));
        $client
            .call_dry_run(&ink_e2e::alice(), &_msg, 0, None)
            .await
            .return_value()
